...
本 Skill 用于让 AI Agent 读取 Taichi 在测试失败后生成的结构化失败上下文（`failure.Context`，下文简称 `Context`），结合服务日志与项目源码，分析失败根因，产出可执行的根因分析报告。
...
## 四、失败上下文格式（failure.Context JSON）
...
```
读取 failure_context_path → 解析 failure.Context JSON
