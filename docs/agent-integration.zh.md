...
taichi 与 AI Agent 的集成是**双向**的，两个方向互补，共同构成「测试 → 分析 → 修复 → 回归」的全自动闭环。失败上下文（`FailureContext`）作为两侧的信息交换契约。
...
- 信息流：taichi → Agent（传递 `FailureContext`）→ Agent 返回 `FixResult` → taichi 应用并回归

### 失败上下文：信息交换契约

无论哪个方向，taichi 与 Agent 之间都以 `FailureContext` JSON 作为失败信息的标准载体，以 `FixResult` JSON 作为修复产出的标准载体。两者定义于：
...
taichi run (测试)
   ...
   ├─ 全部通过 ──► 结束（成功）
   │
   └─ 存在失败 ──► 生成 FailureContext ──► Agent.AnalyzeAndFix
                                                    │
                                                    ▼
                                           应用 FixResult
                                           (git apply / verify)
...
| 工具名 | 用途 | 关键参数 | 返回格式 |
|--------|------|---------|---------|
| `taichi_run` | 执行一次测试编排 | `config`、`project`、`skill[]`、`timeout` | 测试结果摘要（见 3.3） |
| `taichi_list` | 列出配置中的项目、环境与已注册技能 | `config` | 项目/环境/技能清单 |
| `taichi_failures` | 读取最近一次运行的失败上下文 | `config`、`project` | `FailureContext` JSON |
| `taichi_regression` | 执行回归测试（仅 `regression` 技能） | `config`、`project`、`timeout` | 测试结果摘要 |
...
返回最近一次运行的 `FailureContext` JSON（结构见[第七章](#七失败上下文格式)）。若无失败返回空对象。
...
3. **构建失败上下文**：将失败用例封装为 `FailureContext`，写入 `reports/failures-round-<N>-<timestamp>.json`
4. **调用 Agent**：通过 `agent.Invoker` 将 `FailureContext` 交给 AI Agent 分析与修复
5. **应用修复**：
...
通过子进程调用 AI Agent。Agent 脚本需**从 stdin 读取 `FailureContext` JSON，向 stdout 输出 `FixResult` JSON**。
...
契约：
- stdin：`FailureContext` JSON（紧凑格式）
...
契约：
- 请求：`POST <Endpoint>`，`Content-Type: application/json`，body 为 `FailureContext` JSON
...
`FailureContext` 是 taichi ↔ AI Agent 的信息交换契约，定义于 [`pkg/failure/failure.go`](../pkg/failure/failure.go)。
...
- **可序列化**：`FailureContext` 实现 JSON 序列化/反序列化，`ReadFromFile` 可从文件还原
