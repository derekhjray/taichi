...
- 协议：内置三种 — CLI stdin/stdout JSON（`CLIInvoker`）、CLI 提示参数 + 直接编辑文件（`OpenCodeInvoker`）、HTTP API（`HTTPInvoker`）
- 信息流：taichi → Agent（传递 `Context`）→ Agent 返回 `FixResult`（或直接修改文件）→ Taichi 应用并回归
...
```bash
# opencode（直接编辑文件，提示参数协议）
taichi copilot -c configs/taichi.yaml \
  --agent-cli opencode \
  --max-rounds 3

# trae（stdin/stdout JSON 协议）
taichi copilot -c configs/taichi.yaml \
  --agent-cli trae \
...
| `--agent-cli` | AI Agent 命令行可执行文件（如 `opencode`、`trae`、`python3`）；`opencode` 使用直接编辑文件模式，其他使用 stdin/stdout JSON | 必填 |
| `--agent-args` | 传给 Agent 命令的参数（可重复） | 空 |
| `--agent-endpoint` | AI Agent HTTP 端点（与 `--agent-cli` 互斥） | 空 |
| `--agent-token` | HTTP 模式的 Bearer 令牌 | 空 |
| `--agent-timeout` | 单次 Agent 调用超时 | `5m` |
| `--max-rounds` | 最大修复轮次 | `3` |
...
Taichi 通过 `agent.Invoker` 接口抽象 AI Agent 的调用方式，内置三种实现，并支持自定义。
...
### 5.4 OpenCodeInvoker：opencode CLI 调用

调用 [opencode](https://github.com/sst/opencode) CLI 作为 AI Agent。与 `CLIInvoker` 不同，opencode **不使用 stdin/stdout JSON 协议** — 它以人类可读的提示作为命令行参数接收任务，直接修改工作目录中的文件（direct 模式），stdout 为人类可读文本。Taichi 在调用结束后通过 `git status --porcelain` 检测被修改的文件（非 git 项目则回退到基于 mtime 的扫描）。

```go
invoker := &agent.OpenCodeInvoker{
    Command: "opencode",                 // opencode 二进制路径，空默认为 "opencode"
    Args:    []string{"--model", "anthropic/claude-3.5-sonnet"}, // `opencode run` 的额外参数
    Timeout: 15 * time.Minute,           // 单次调用超时，0 表示默认 15 分钟
    WorkDir: "/path/to/project",         // 覆盖 fc.ProjectRoot
}
```

契约：
- 参数：`opencode run [额外参数] "<提示>"` — Taichi 从 `Context` 构建提示
- stdout/stderr：继承父进程（用户可实时查看 opencode 的进展）
- stdin：不使用（opencode 在 run 模式下不读 stdin）
- 文件修改：opencode 直接在 `WorkDir` 中编辑文件
- 检测：Taichi 在调用前快照 `git status`，调用后对比，仅将新增变更归因于 opencode（预先存在的脏文件被排除）
- 非 git 回退：Taichi 扫描 mtime ≥ 快照时间点的源码文件
- 退出码：非零视为调用失败

使用 `taichi copilot` 时，opencode 二进制由 `--agent-cli` 按命令 basename 自动识别（如 `opencode`、`/usr/local/bin/opencode`、`opencode.exe` 均可匹配），无需额外标志：
...
子目录路径处理：当项目根是一个更大 git 仓库的子目录时，`git status --porcelain` 报告的路径相对于仓库根。`OpenCodeInvoker` 通过 `git rev-parse --show-prefix` 获取仓库根到当前目录的前缀并剥离，使路径重新变为项目根相对，从而下游 `VerifyDirectFix` 可正确定位。

> 实现：[`pkg/agent/opencode.go`](../pkg/agent/opencode.go)。自动识别：同文件中的 `OpenCodeDetect`。接入 `taichi copilot` 通过 [`cmd/taichi/copilot.go`](../cmd/taichi/copilot.go) 中的 `buildInvoker`。
